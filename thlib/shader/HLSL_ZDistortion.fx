...
PS_OUTPUT PsWave( PS_INPUT In ) : COLOR0
{
	...
	float angle = In.vPos.y - 0 + In.vPos.x - 0 + frame_ * 4;
	float angleB = In.vPos.y - 0 + In.vPos.x - 0 + frame_ * 3;
	angle = radians(angle);
	angleB = radians(angleB);

	//該当ピクセルの歪みの半径を計算
	//エフェクト半径の1/16を最大の歪み幅とする
	float waveRadius = waveRadius_ + waveRadius_/50 * (-1 + sin(angle*3)*2) * (-1 + sin(angleB*4)/2);
